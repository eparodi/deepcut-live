...
instructing you to. Stick to `backend/`.

### Prerequisites for the user

The user must have `go` installed and the project dependencies set up
(run `go mod download` once). No extra global tools are required for
this role beyond what the orchestrator needs.
