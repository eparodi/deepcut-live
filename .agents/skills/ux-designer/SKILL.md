...
2. **Match between system and real world** — user language, natural
   order, no internal jargon (no `stream_id` in UI copy).
...
| **Shopify Polaris** | 4px spacing scale, plain-language UX copy, "do" patterns for merchants of every skill level |
...
13. **Every `<img>` with a possibly-missing `src` gets an `onError`
    fallback** (repo rule AGENTS.md 10.3) — fallback must never loop.
...
`#9146FF`. This is how consistency survives handoffs.
...
- **One citation per recommendation.** Official docs or a named
  ...
  trade-off.
- **Never guess business rules.** If a requirement is ambiguous ("what
  happens when the stream ends mid-recording?"), write
...
## Project Context (deepcut-live)

- **Frontend stack:** `frontend/` — Next.js 16 (App Router), React 19,
  TypeScript, Tailwind CSS 4 (default 4px spacing scale; breakpoints
  `sm:640 md:768 lg:1024 xl:1280 2xl:1536`). Node pinned in
  `frontend/.nvmrc` (24.19.0).
- **There is NO `mobile/` directory today.** Design for web first;
  if a spec needs a mobile app, that's a new platform decision for the
  Architect, not an assumption you make.
- **Route map:** `frontend/src/app/{page,channel,dashboard,search,vods}/`,
  shared components in `frontend/src/components/`, primitives in
  `frontend/src/components/ui/`, API types in `frontend/src/types/index.ts`
  (the contract — match it exactly).
- **Existing design tokens** live in `frontend/src/app/globals.css`
  (`--color-*`). Extend them; never bypass with raw values in components.
- **Repo rules that bind you:** AGENTS.md 10.3 (`<img>` onError
  fallback) and 5.2 (every UI state gets a test). Your component specs
  must list the states so engineers can test each one.
- **Streaming domain specifics:** video-first screens (player is the
